...
public class ReportStoreMenu extends Panel {
	...
	public ReportStoreMenu(MainLayout mainLayout) {
		...
		dataset = createSalesDataset(year);
		setCombobox();
	...
	public XYDataset createSalesDataset(int year) {
		...
		// 유저가 가진 지점 정보만큼 반복하며 데이터명 설정
		for (Branch branch : brList) {
			salesList = stockDAO.selectSales(branch.getBr_id()); // 해당 지점 매출 리스트 저장
			
			// 지점 한 개 생성 시작 (이름 설정)
			XYSeries series = new XYSeries(branch.getBr_name());

			// 1~12월 매출 0으로 초기화
			Map<Integer, Integer> salesMap = new HashMap<>(); // 1~12월까지의 데이터를 세팅할 맵 선언
			for (int i = 1; i <= 12; i++)
				salesMap.put(i, 0); // 누락된 월 방지 초기화

			// 특정 연도의 DB 값 불러와 재설정
			for (Map<String, Integer> sale : salesList) {
				if (sale.get("Year") == year) {
					int month = sale.get("Month");
					int sales = sale.get("Sales");
					salesMap.put(month, sales / 10000);
				}
			}

			// 차트 데이터셋에 추가
			for (int i = 1; i <= 12; i++)
				series.add(i, salesMap.get(i));
